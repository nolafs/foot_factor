...
const AutocompleteItem = ({ item }: { item: HitBaseItem }) => {
  ...
  return (
    <div className="group relative flex w-full items-center justify-between space-x-4 border-b border-gray-200 p-4 transition-colors hover:bg-gray-50">
      ...
      <div className="flex grow flex-col space-y-2">
        <Badge className={'w-fit bg-accent text-[9px] capitalize'}>{item.category}</Badge>
    ...
    <div className="relative" ref={dropdownRef}>
      <Button
        ...
        className={cn(
          ...
          selectedCount > 0 ? 'border-accent bg-white text-accent' : 'border-gray-300 hover:border-gray-400',
        )}>
        <Filter className="h-4 w-4" />
        {selectedCount > 0 && (
          <div className="absolute right-1 top-1 flex h-4 w-4 items-center justify-center rounded-full bg-accent text-[10px] text-white">
      ...
      {isOpen && (
        <div className="absolute right-0 top-full z-50 mt-2 max-h-60 w-64 overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-lg">
          ...
          {items.map(item => (
            <label
              key={item.value}
              className="flex cursor-pointer items-center space-x-3 border-b border-gray-50 px-4 py-2 last:border-b-0 hover:bg-gray-50">
              <input
                type="checkbox"
              ...
              <span className="rounded bg-gray-100 px-2 py-1 text-xs text-gray-500">{item.count}</span>
            </label>
          ))}
        </div>
      )}
...
const SearchComponent = () => {
  ...
  return (
    <div className="relative z-10 mb-10 flex w-full flex-col">
      ...
      <div className="relative mt-4 flex space-x-2">
        <div className="relative grow">
          ...
          <input
            ...
            className="w-full rounded-lg border border-gray-300 py-3 pl-10 pr-10 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
          {inputValue && (
            <button
              onClick={handleClear}
      ...
      {/* Results */}
      {isOpen && inputValue && (
        <div className="absolute z-[999] mt-28 w-full overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg">
          {/* Stats */}
          {inputValue && (
            <div className="border-b border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-500">
              <Stats />
            </div>
          )}
...
export const AutoCompleteTreatments = () => {
  ...
  return (
    <InstantSearch searchClient={searchClient} indexName="treatments">
      <Configure hitsPerPage={8} analytics={true} clickAnalytics={true} />
      <SearchComponent />
    </InstantSearch>
  );
};
