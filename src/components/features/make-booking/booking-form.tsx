...
export const BookingForm = ({ booking }: BookingFormProps) => {
  ...
  const isReferralPatient = useWatch({
  ...
  if (submissionSuccess) {
    return (
      <div className="mt-10">
        <h2 className="mb-4 text-2xl font-bold">Booking Request Submitted</h2>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex items-center space-x-3">
                ...
                <FormLabel className="font-normal">Are you an existing patient?</FormLabel>
              </FormItem>
            )}
          />
        </div>
                ...
                <Popover modal open={open} onOpenChange={setOpen}>
                  <PopoverTrigger asChild className="w-full">
                    <Button
                      type="button"
                      variant="ghost"
                      className="absolute right-2 top-1/2 size-6 -translate-y-1/2 !bg-transparent hover:text-gray-900"
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                ...
                <SelectContent>
                  {booking.appointment_type.map((appointment, index) => (
                    <SelectItem key={index} value={appointment.value ?? ''} className={'text-sm'}>
                      {appointment.label}
                    </SelectItem>
                  ))}
                </SelectContent>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem className="flex items-center space-x-3">
                ...
                <FormLabel className="font-normal">Referral patient?</FormLabel>
              </FormItem>
            )}
          />
        </div>
          ...
          render={({ field }) => (
            <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
              ...
              <div className="space-y-1 leading-none">
                ...
                <FormDescription>
                  Please read our{' '}
                  <Link className={'underline'} href="/legal/terms-and-conditions">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
              ...
              <div className="space-y-1 leading-none">
                <FormLabel>Agree to our privacy policy</FormLabel>
                <FormDescription>
                  Please read our{' '}
                  <Link className={'underline'} href="/legal/privacy-policy-for-foot-factor">
                    Privacy Policy
                  </Link>{' '}
                  page.
                </FormDescription>
