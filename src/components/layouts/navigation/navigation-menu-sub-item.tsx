            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              {item.subs.map((sub, idx) => (
                <div
                  ...
                  <PrismicNextLink field={sub.link} className="flex flex-col space-y-1">
                    <div className="text-base font-medium text-gray-800 transition-all group-hover:text-white">
                      {sub.label}
                    </div>
                    <div className="text-gray-400 transition-all group-hover:text-white">{sub.subtitle}</div>
          ...
          <div className="relative col-span-3">
            {item.subs.map((sub, idx) => (
              <motion.div
                ...
                key={idx}
                initial={{ opacity: 0 }}
                animate={{ opacity: activeIndex === idx ? 1 : 0 }} // ← use derived index
              >
                <div className="relative">
                  <div className={cn('aspect-h-9 aspect-w-16 overflow-hidden rounded-xl outline-1 -outline-offset-1')}>
                    <PrismicImage field={sub.image} className="block size-full object-cover object-center" />
                  </div>
                </div>
                <div
                  className={cn(
                  ...
                  <h3 className="mb-2 flex space-x-2 text-lg font-bold text-black">
                    <div className="aspect-1 left-2 top-2 z-20 flex size-6 items-center justify-center rounded-lg bg-black">
                      <PrismicImage field={sub.icon} className="size-4 invert" />
                    </div>
                    <span>{sub.label}</span>
                  </h3>
                  <div className="text-xs font-medium text-black/40">
                    <PrismicRichText field={sub.description} />
                  </div>
                </div>
              </motion.div>
            ))}
