...
export async function generateMetadata(
  ...
  return {
    metadataBase: new URL(`${process.env.NEXT_PUBLIC_BASE_URL}/conditions/${cat}`),
    alternates: {
      canonical: `${process.env.NEXT_PUBLIC_BASE_URL}/conditions/${cat}`,
      types: {
      ...
      <Container as={'section'} padding={'lg'}>
        ...
        <ul className={'grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4'}>
          {conditions.map((item, idx) => (
            <li key={idx}>
              <Link
                href={`/conditions/${(item.data?.category as { uid: string }).uid}/${item.uid}`}
                className={
                  'group flex flex-col gap-4 rounded-lg bg-white p-4 transition-shadow duration-300 hover:shadow-lg'
                }>
                {isFilled.image(item.data.feature_image) ? (
                  <PrismicNextImage field={item.data.feature_image} className={'h-48 w-full rounded-lg object-cover'} />
                ) : (
                  <Image
                    src={PlaceholderImage}
                    alt={item.data.title ?? 'placeholder image'}
                    className={'h-48 w-full rounded-lg object-cover'}
                  />
                )}
                <div>
                  <Badge variant={'secondary'}>{(item.data?.category as { data: { name: string } }).data.name}</Badge>
                </div>
                <div>
                  <h3 className={'mb-3 text-lg font-semibold group-hover:text-accent'}>{item.data.title}</h3>
