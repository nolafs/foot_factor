...
export async function generateMetadata(
  ...
  const page = await client.getByUID('treatment', uid).catch(() => notFound());
  ...
  return {
    metadataBase: new URL(`${process.env.NEXT_PUBLIC_BASE_URL}/treatments/${page.uid}`),
    alternates: {
      canonical: `${process.env.NEXT_PUBLIC_BASE_URL}/treatments/${page.uid}`,
      types: {
...
export default async function Page({ params }: { params: Promise<Params> }) {
  ...
  return (
    <main className={'w-full overflow-hidden'}>
      <HeroSimple
        wave_type={page.data.wave as WaveType}
        heading={page.data.heading}
        label={'treatment'}
        subheading={page.data.lead}
      />
...
export async function generateStaticParams() {
  const client = createClient();
  const pages = await client.getAllByType('treatment');
